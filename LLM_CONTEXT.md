...
## Current business rule map
 ...
 - Regra: A interface desktop (Tauri) e a interface web (Fastify static) aceitam 1 ou vários arquivos por ação via drag-and-drop ou seletor. Com múltiplos arquivos, o desktop cria um ZIP temporário via Rust e o cliente web cria um ZIP via `fflate` em JS. Ambos enviam o pacote final como 1 upload para `POST /api/upload`, gerando 1 link público. Fonte: UI `src/desktop/App.tsx`; `src/desktop/tauri.ts`; comandos Tauri `src-tauri/src/lib.rs`; Backend `src/server/upload-service.ts`.
...
- Desktop Rust commands: `src-tauri/src/lib.rs` implements `upload_file`, `upload_files`, `copy_link`, `notify_success`; `upload_files` validates selected paths, creates a temporary `.zip` with unique entry names when there is more than 1 file, uploads that ZIP through the existing single-file backend endpoint, then removes the temp file. Config reads `QUICKDROP_API_BASE_URL` with default `http://127.0.0.1:3000`.
 - Desktop UI / Web Frontend: `src/desktop/App.tsx` funciona em ambos os ambientes (detectado via `isTauri`). No desktop usa drag-drop events do Tauri e seletor nativo; na web usa HTML5 drag-and-drop, input de arquivos e Clipboard API padrão. Mostra progresso real do upload e fase `Criando ZIP...` ao compactar múltiplos arquivos.
