...
describe("install-macos.sh", () => {
  ...
  test("restores the previous app when the final replacement fails", async () => {
    const root = await mkdtemp(join(tmpdir(), "quickdrop-macos-rollback-test-"));
    temporaryDirectories.push(root);
    const mockBin = join(root, "mock-bin");
    const home = join(root, "home");
    const applications = join(root, "Applications");
    const installedExecutable = join(applications, "QuickDrop.app", "Contents", "MacOS", "quickdrop");
    const sourceQd = join(root, "qd");
    const sourceDmg = join(root, "QuickDrop.dmg");
    const mvState = join(root, "mv-state");
    await mkdir(mockBin, { recursive: true });
    await mkdir(home, { recursive: true });
    await mkdir(join(applications, "QuickDrop.app", "Contents", "MacOS"), { recursive: true });
    await writeFile(installedExecutable, "previous app");
    await executable(sourceQd, "#!/bin/sh\nexit 0\n");
    await writeFile(sourceDmg, "local test dmg");
    ...
    await executable(
      join(mockBin, "hdiutil"),
      '#!/bin/sh\nif [ "$1" = attach ]; then while [ "$#" -gt 0 ]; do if [ "$1" = -mountpoint ]; then shift; mount_dir="$1"; fi; shift; done; mkdir -p "$mount_dir/QuickDrop.app/Contents/MacOS"; printf replacement > "$mount_dir/QuickDrop.app/Contents/MacOS/quickdrop"; fi\n',
    );
    await executable(join(mockBin, "ditto"), '#!/bin/sh\ncp -R "$1" "$2"\n');
    await executable(
      join(mockBin, "mv"),
      `#!/bin/sh
count=0
[ ! -f "${mvState}" ] || count=$(cat "${mvState}")
count=$((count + 1))
printf '%s' "$count" > "${mvState}"
[ "$count" -ne 2 ] || exit 1
exec /bin/mv "$@"
`,
    );
