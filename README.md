...
- `POST /api/text/:code/access` — valida o PIN da sala protegida (ou reaproveita um cookie ainda válido), define/renova o cookie HttpOnly por sala e libera a entrada. Sala pública responde sem exigir PIN.
...
- `WS /api/text/:code/ws` — o servidor envia `snapshot` ao conectar, `update` quando outro cliente escreve, `presence` com a contagem de clientes conectados, `ack` ao autor após cada escrita e `error` (texto acima do limite, sala inválida ou acesso protegido sem PIN). O cliente envia `{ type: "write", text, baseVersion }`. Heartbeat ping/pong derruba conexões mortas.
