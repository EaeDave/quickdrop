...
- `WS /api/text/:code/ws` — o servidor envia `snapshot` ao conectar, `update` quando outro cliente escreve, `presence` com a contagem de clientes conectados, `ack` ao autor após cada escrita e `error` (texto acima do limite, sala inválida ou acesso protegido sem PIN). Eventos legados `typing`, `pointer` e `peer_left` continuam aceitos/transmitidos para compatibilidade, mas a interface web de clipboard não os envia nem os exibe. Heartbeat ping/pong derruba conexões mortas.
- O texto persistido usa uma janela de envio curta (~75 ms) para reduzir a sensação de atraso sem abandonar o modelo atual last-writer-wins; a interface permite limpar o conteúdo para todos os clientes com confirmação. Conteúdo remoto aplicado recebe destaque e anúncio acessível, sem alterar foco ou seleção; o destaque é removido quando o usuário copia ou edita o texto.
